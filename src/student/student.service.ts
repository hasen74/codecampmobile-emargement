...
export class StudentService {
  ...
  async setPassword(
    ...
    if (!existingStudent) {
      throw new NotFoundException(`Student ${login} not found`);
    }
  ...
  async getStudentByLogin(login: string): Promise<IStudent> {
    const existingStudent = await this.studentModel.findOne({login: login}).exec();
    if (!existingStudent) {
      throw new NotFoundException(`Student ${login} not found`);
    }
    return existingStudent;
